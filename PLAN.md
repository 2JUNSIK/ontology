...
> N11 `feat-graph-visualization`을 ff 머지). 이전 문서에 'N9·N10은 `feat-ontology-enrichment`에 PR 예정'으로
> 적혀 있었으나 `git rev-parse` 확인 결과 이미 머지됐었음. **N12는 `feat-query-presets`**(탐색 예시 질문 프리셋)에
